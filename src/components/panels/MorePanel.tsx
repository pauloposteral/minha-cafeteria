  ...
  return (
    <div className="flex flex-col h-full bg-cafe-50">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-2">
        ...
        >
          <BarChart3 size={20} className="text-cafe-500" />
          <div>
        ...
        <button
          ...
          className={`w-full flex items-center gap-3 p-3 rounded-xl border transition-all text-left shadow-sm ${
            canPrestige
              ? 'bg-cafe-300/10 border-cafe-300/50 hover:border-cafe-300'
              : 'bg-white border-cafe-200 hover:border-cafe-300'
          }`}
        >
          <Award size={20} className={canPrestige ? 'text-cafe-400' : 'text-cafe-400'} />
          <div>
        ...
        <button
          ...
          className="w-full flex items-center gap-3 p-3 bg-white rounded-xl border border-cafe-200 hover:border-cafe-300 transition-all text-left shadow-sm"
        >
          <Star size={20} className="text-cafe-400" />
          <div>
        ...
        <button
          ...
          className="w-full flex items-center gap-3 p-3 bg-white rounded-xl border border-cafe-200 hover:border-cafe-300 transition-all text-left shadow-sm"
        >
          <Settings size={20} className="text-cafe-500" />
          <div>
        ...
        {state.prestige.cities.length > 1 && (
          <div className="bg-white rounded-xl p-3 border border-cafe-200 shadow-sm">
            <p className="text-xs text-cafe-500 mb-1">Suas cidades:</p>
            <div className="flex flex-wrap gap-1">
              {state.prestige.cities.map((city, i) => (
                <span key={i} className="text-xs bg-cafe-100 text-cafe-600 px-2 py-0.5 rounded-full border border-cafe-200">
                  {city}
                </span>
              ))}
            </div>
      ...
      <Modal open={openSection === 'stats'} onClose={() => setOpenSection(null)} title="Estatisticas">
        <div className="space-y-3">
          <StatRow icon={<Coffee size={14} className="text-cafe-500" />} label="Drinks servidos" value={formatNumber(state.stats.totalDrinksServed)} />
          <StatRow icon={<Users size={14} className="text-cafe-500" />} label="Clientes atendidos" value={formatNumber(state.stats.totalCustomersServed)} />
          <StatRow icon={<Coins size={14} className="text-cafe-500" />} label="Total ganho" value={formatMoney(state.stats.totalMoneyEarned)} />
          <StatRow icon={<TrendingUp size={14} className="text-cafe-500" />} label="Gorjetas totais" value={formatMoney(state.stats.totalTipsEarned)} />
          <StatRow icon={<Star size={14} className="text-cafe-400" />} label="Melhor dia" value={formatMoney(state.stats.bestDayEarnings)} />
          <StatRow icon={<Clock size={14} className="text-cafe-500" />} label="Tempo de jogo" value={formatDuration(state.stats.playTime * 1000)} />
          {mostPopularRecipe && (
      ...
      <Modal open={openSection === 'prestige'} onClose={() => setOpenSection(null)} title="Prestigio">
        <div className="space-y-4">
          <div className="text-center">
            ...
            <p className="text-cafe-800 font-bold">Prestigio {state.prestige.level}</p>
            <p className="text-xs text-cafe-500">Multiplicador atual: x{state.prestige.permanentMultiplier}</p>
          </div>

          {canPrestige ? (
            <div className="bg-cafe-300/10 rounded-xl p-3 border border-cafe-300/30">
